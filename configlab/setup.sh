#!/usr/bin/env bash
# =============================================================
# configlab v2 — fresh-install bootstrap
...
APP_DIR="/var/www/configlab"
APP_USER="${SUDO_USER:-$USER}"
DB_NAME="configlab_db"
DB_USER="configlab_user"
DB_PASS="$(openssl rand -base64 24)"
DOMAIN="configlab.example.com"   # used for self-signed cert CN
PORT=3000
...
cat > "${APP_DIR}/.env" <<ENV
# configlab v2 — generated by setup.sh on $(date -u '+%Y-%m-%dT%H:%M:%SZ')
...
mkdir -p /var/log/configlab
chown "${APP_USER}:${APP_USER}" /var/log/configlab

sudo -u "${APP_USER}" pm2 start "${APP_DIR}/ecosystem.config.js" || \
sudo -u "${APP_USER}" pm2 restart configlab-app
...
openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
    ...
    -subj   "/CN=${DOMAIN}/O=configlab/C=GB" 2>/dev/null
chmod 600 /etc/ssl/private/configlab.key

# ── 10. Nginx ─────────────────────────────────────────────────
info "Configuring Nginx…"
cat > /etc/nginx/sites-available/configlab <<NGINX
# HTTP → HTTPS redirect
server {
    listen 80;
    listen [::]:80;
...
ln -sf /etc/nginx/sites-available/configlab /etc/nginx/sites-enabled/configlab
...
cat > /usr/local/bin/backup-configlab-db.sh <<'BACKUP'
#!/bin/bash
...
BACKUP
chmod +x /usr/local/bin/backup-configlab-db.sh

# Add daily cron job for postgres user
(crontab -l -u postgres 2>/dev/null || true; echo "0 2 * * * /usr/local/bin/backup-configlab-db.sh") \
    | sort -u | crontab -u postgres -

# ── Done ──────────────────────────────────────────────────────
echo ""
echo "════════════════════════════════════════════════════════"
echo -e "${GREEN}  configlab v2 installed successfully!${NC}"
echo "════════════════════════════════════════════════════════"
echo ""
...
echo "    pm2 status"
echo "    pm2 logs configlab-app"
echo "    pm2 restart configlab-app"
